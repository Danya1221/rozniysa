...
RETAIL_BUILD = "retail-2026.09.21-recovery"
...
class RetailSyncService(SyncService):
    ...
    def retail_cached_items(self):
        ...
        for slot in (1, 2):
            ...
            for source in self._slot_sources(slot):
                value = cache.get(self.source_cache_key(source, slot), {})
                # An empty successful snapshot is still authoritative: it must
                # not resurrect stock from an explicitly closed supplier.
                has_open_source = has_open_source or value.get("status") == "open"
        ...
        return merge_lowest(open_groups if has_open_source else fallback_groups)
